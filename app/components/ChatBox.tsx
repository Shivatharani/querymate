  ...
  return (
    <div className="flex items-start gap-3 p-4">
      <div className="flex h-7 w-7 items-center justify-center rounded-full bg-gray-200">
        <div className="h-2.5 w-2.5 animate-ping rounded-full bg-black" />
      </div>
...
function FilePreview({ file, onRemove }: { file: File; onRemove: () => void }) {
  return (
    <div className="group flex items-center gap-2 rounded-lg border border-gray-300 bg-white p-2 shadow-sm hover:shadow-md transition-all">
      <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-md border border-gray-300 bg-gray-50">
      ...
      <button
        type="button"
        onClick={onRemove}
        className="opacity-0 group-hover:opacity-100 h-7 w-7 flex-shrink-0 ml-1 items-center justify-center rounded-md border border-gray-300 bg-white hover:bg-gray-100 transition-all"
...
function CompactTokenStatus({ tokenStatus }: { tokenStatus: TokenStatus | null }) {
  ...
  return (
    <div className="flex items-center justify-between gap-2 border-b border-gray-200 px-4 py-2.5 bg-gray-50">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="w-8 text-right font-mono text-xs">{percentage}%</span>
      </div>
    </div>
  )
}
      ...
      <div className="space-y-1.5">
        {sources.map((source, index) => (
          <a
            ...
            className="block p-2.5 rounded-lg border border-gray-200 hover:bg-gray-50 hover:border-black transition-all text-sm truncate"
          >
            {source.title}
          </a>
        ))}
...
export default function ChatBox({
  ...
  return (
    <div className="flex h-screen w-full flex-col overflow-hidden bg-white">
          ...
          >
            {showCenterPrompt ? (
              <div className="flex flex-1 flex-col items-center justify-center gap-8 text-center px-4 max-w-md mx-auto">
                ...
                {/* Single row suggestions - no scroll */}
                <div className="w-full flex flex-wrap gap-2.5 justify-center max-w-lg">
                  {suggestions.map((suggestion) => (
                    <button
                      key={suggestion}
                      onClick={() => handleSuggestionClick(suggestion)}
                      className="px-4 py-2.5 text-xs font-medium bg-white border border-gray-300 rounded-xl hover:bg-gray-50 hover:border-gray-400 hover:shadow-sm transition-all duration-200 backdrop-blur-sm flex-shrink-0 min-w-[100px] max-w-[200px]"
                    >
                      {suggestion}
                    </button>
                  ))}
                    ...
                    {m.role === 'user' ? (
                      <div className="flex w-full justify-end">
                        <div className="flex flex-col items-end max-w-xs sm:max-w-md lg:max-w-lg">
                          <div className="bg-gray-200 rounded-2xl rounded-br-md px-4 py-3 max-w-full border border-gray-300">
                            <div className="prose prose-sm max-w-none text-black leading-relaxed break-words">
                              {m.content}
                            </div>
                            {m.files && m.files.length > 0 && (
                              <div className="mt-2.5 flex flex-wrap gap-1.5 pt-1.5 border-t border-gray-300">
                                {m.files.map((file, idx) => (
                                  <div
                                    key={idx}
                                    className="flex items-center gap-1.5 px-2 py-1 bg-white rounded-lg border border-gray-300 text-xs"
                                  >
                          ...
                          <div className="bg-white rounded-2xl px-4 py-3 shadow-sm border border-gray-200">
                                ...
                                components={{
                                  a: ({ node, children, href, ...props }) => (
                                    <a
                                      ...
                                      target="_blank"
                                      rel="noopener noreferrer"
                                      className="text-black underline hover:text-gray-900 break-all font-medium"
                                      {...props}
                                    >
                                      {children}
                                    </a>
                                  ...
                                  code({ className, children, ...props }) {
                                    ...
                                    return (
                                      <div className="my-4 rounded-xl border border-gray-300 bg-gray-50 shadow-sm overflow-hidden">
                                        <div className="flex items-center justify-between px-4 py-2.5 border-b border-gray-200 bg-white">
                                          ...
                                          <CodeBlockCopyButton
                                            ...
                                            className="h-7 w-7 rounded-lg bg-white border border-gray-300 text-black hover:bg-gray-100 transition-all"
                                          />
                                        </div>
                                  ...
                                  pre: () => null,
                                  p: ({ children, ...props }) => (
                                    <p className="mb-3 leading-relaxed text-black" {...props}>
                                      {children}
                                    </p>
                                  ),
                                  ul: ({ children, ...props }) => (
                                    <ul className="list-disc ml-6 space-y-1.5 mb-3" {...props}>
                                      {children}
                                    </ul>
                                  ),
                                  ol: ({ children, ...props }) => (
                                    <ol className="list-decimal ml-6 space-y-1.5 mb-3" {...props}>
                                      {children}
                                    </ol>
                                  ),
                                  li: ({ children, ...props }) => (
                                  ...
                                  h2: ({ children, ...props }) => (
                                    <h2 className="text-lg font-bold mt-5 mb-2.5 text-black" {...props}>
                                      {children}
                                    </h2>
                                  ),
                                  h3: ({ children, ...props }) => (
                                    <h3 className="text-base font-bold mt-4 mb-2 text-black" {...props}>
                                      {children}
                                    </h3>
                                  ),
                                }}
                            ...
                            <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-200">
                              <button
                                ...
                                className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-100 hover:bg-gray-200 rounded-xl text-xs font-medium text-black border border-gray-300 transition-all"
                              >
      ...
      {/* Input area */}
      <div className="border-t border-gray-300 bg-white px-4 py-4 sm:px-6 sticky bottom-0">
        <div className="mx-auto max-w-2xl w-full">
          ...
          {errorMessage && (
            <div className="mb-3 p-3 rounded-xl bg-gray-50 border border-gray-300 text-sm text-black">
              {errorMessage}
            </div>
          )}

          <form onSubmit={handlePromptSubmit} className="flex items-end gap-2">
            <div className="flex flex-1 flex-col gap-2 p-3 bg-gray-50 border border-gray-300 rounded-2xl shadow-sm hover:shadow-md transition-all">
              <textarea
                value={input}
                ...
                rows={1}
                disabled={isTyping}
                className="w-full resize-none bg-transparent border-none p-0 text-sm font-normal text-black outline-none placeholder-gray-500 disabled:opacity-50 min-h-[20px] max-h-24 leading-relaxed"
              />
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  <button
                    ...
                    disabled={isTyping}
                    className="h-10 w-10 rounded-xl bg-white border border-gray-300 hover:bg-gray-50 flex items-center justify-center shadow-sm hover:shadow-md transition-all disabled:opacity-50"
                  ...
                  <button
                    ...
                    className={`h-10 w-10 rounded-xl flex items-center justify-center shadow-sm transition-all disabled:opacity-50 ${
                      isListening
                        ...
                        : 'bg-white border border-gray-300 hover:bg-gray-50 hover:shadow-md text-black'
                    }`}
                  ...
                  <PromptInputSelect
                    ...
                    disabled={isTyping}
                  >
                    <PromptInputSelectTrigger className="h-10 w-28 rounded-xl bg-white border border-gray-300 px-2.5 shadow-sm hover:shadow-md text-xs font-medium">
                      Model
                    </PromptInputSelectTrigger>
                ...
                <button
                  ...
                  className={`h-12 px-4 rounded-xl flex items-center gap-2 font-semibold transition-all shadow-lg ${
                    isTyping || !input.trim()
                      ...
                      : 'bg-black hover:bg-gray-900 text-white hover:shadow-xl hover:-translate-y-0.5 active:translate-y-0'
                  } min-w-[56px]`}
          ...
          <p className="text-xs text-gray-500 text-center mt-3 pt-2 border-t border-gray-300">
